...
int TickFct_Joystick(int state) {
    switch(state) { // Transitions
        ...
        case STICK_BUTTON_RELEASE:
            ...
            break;
        case STICK_WAIT:
            if (buttonTwo/*coords[1] < JOYSTICK_INIT - DELTA*/) {
            ...
            else if (buttonThree/*coords[1] > JOYSTICK_INIT + DELTA*/) {
                if (characterCursorPos == 1) {
                    characterCursorPos = 17;
                    state = STICK_DOWN;
                }
                else
                    state = STICK_WAIT;
...
int main() {
    ...
    while(1) {
        buttonOne = (~PINA) & 0x80;        //PA7
        buttonTwo = (~PINA) & 0x10;        //PA4
        buttonThree = (~PINA) & 0x04;    //PA2
        buttonFour = (~PINA) & 0x01;    //PA0
    } // while (1)
